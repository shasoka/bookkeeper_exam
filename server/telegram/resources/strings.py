...
CHANGELOGS = [
    ...
    \n\n🎓 Начать экзамен можно командой /exam. Я выберу для 35 случайных вопросов и дам тебе 20 минут на их решение. Все просто, но я старался! Надеюсь, ты оценишь.
    \n\n⚠️ Тем, кто не пользовался мной несколько дней, напоминаю: Телеграм не даст удалить мне некоторые старые сообщения, поэтому, если получите сообщение об ошибке ❌🧹, воспользуйтесь командой /heal.
    \nКак обычно, если что-то сломается, пиши @shasoka
    \nМожешь спокойно удалять это сообщение 🫥
    \nУспехов! 🤞""",
...
def on_exam_end(record: bool, score: int) -> str:
    return f"""
        Ладно, ладно. Успокойся. Финиш 🏁
        \n{'Так уж и быть, обрадую. Ты побил свой предыдущий рекорд! ✳️ ' if record else 'Даже лучшие порой ошибаются... Но это тоже хороший результат! *️⃣ '} Правильных ответов: {html.code(str(score) + '/35')}
