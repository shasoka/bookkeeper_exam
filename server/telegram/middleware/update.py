        ...
        if event.from_user:
            ...
            if not user.checked_update:
                await event.answer(
                    ...
                    \n\n📖 2. Я решил перечитать методичку с тестами, чтобы исправить {html.spoiler(html.italic('ВОТТАКИЕВОТСЛИПШИЕСЯСЛОВА') + ' 🧲')} и {html.spoiler(html.italic('ПЕРЕ-НОСЫ ПО СЛО-ГАМ') + ' ✂️')}
