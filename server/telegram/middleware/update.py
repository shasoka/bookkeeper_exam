        ...
        if event.from_user:
            ...
            if not user.checked_update:
                await event.answer(
                    ...
                    \n🛠 1. Меня подлатали {html.strikethrough('снова')} и исправили несколько багов 🪲, которые случались при обращении к функции /heal, во время моих попыток удалить некоторые сообщения, а также в ситуациях, когда новый пользователь не может получить ко мне доступ из-за отсутствия у него @имени_пользователя_в_tg.
                    \n\n📖 2. Меня вынудили перечитать методичку с тестами {html.italic(html.strikethrough('снова'))}, чтобы исправить {html.bold('два')} вопроса, в одном из которых слиплись варианты ответа (что-то вроде "а) ...; б) ...;"), а в другом отсутствовал верный ответ {html.italic(html.spoiler('его там все еще нет, но скоро это исправится, возможно, даже с твоей помощью'))}.
